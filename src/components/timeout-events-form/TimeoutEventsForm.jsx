...
import TimeoutEventForm from './timeout-event-form/TimeoutEventForm';

const TimeoutEventsForm=({instance, updateInstance})=>{
    ...
    return(
        <Fragment>
            <div style={{
                ...
                <h5>排程觸發事件</h5>
                {
                    (instance?.timeout_events||[]).length>0 ?
                    (instance?.timeout_events||[]).map((timeoutEvent,key)=>
                        {
                            const setTimeoutEvent = (data)=>{
                                updateInstance({...instance, timeout_events:(instance?.timeout_events||[]).map((_timeout_event, _index)=>_index==key?data:_timeout_event)})
                            }

                            const removeTimeoutEvent = ()=>{

                                console.log(key)
                                updateInstance({...instance, timeout_events:(instance?.timeout_events||[]).filter((_timeout_event, _index)=>_index!=key)})
                            }
                    
                            return (<TimeoutEventForm key={key}

                                timeoutEvent={timeoutEvent}
                                setTimeoutEvent={setTimeoutEvent}
                                removeTimeoutEvent={removeTimeoutEvent}
                            />)
                        }
                    )
                    :
                ...
                <Button 
                    variant="secondary" 
                    onClick={(e)=>{
                        updateInstance({...instance, timeout_events:[...(instance?.timeout_events||[]), {timeout:null, emit_event_name:'' }]})
...
export default TimeoutEventsForm;
