...
export async function getBlogPostDetail(blog_post_uuid){
    if(blog_post_uuid){
        ...
        const res = await fetch(`${process.env.API_PROTOCAL}://${process.env.API_HOSTNAME}${process.env.API_PORT?':'+process.env.API_PORT:''}/api/v1/store/blog_post/${blog_post_uuid}/retrieve/`,
        { 
            method: 'GET',
            headers: {
                // 'Authorization': `Bearer ${cookieStore.get('customer_access_token')?.value}`,
                'Next-Server-Origin': 'localhost:3000'
